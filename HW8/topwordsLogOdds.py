...
def calculate_log(prob_con_table, prob_lib_table):
    log_table_l_c = {}
    log_table_c_l = {}
    for word in prob_lib_table:
        log_table_l_c[word] = math.log(prob_lib_table[word]) - math.log(prob_con_table[word])
        log_table_c_l[word] = math.log(prob_con_table[word]) - math.log(prob_lib_table[word])
    return log_table_l_c, log_table_c_l
...
if len(sys.argv) != 2 or not os.path.isfile(sys.argv[1]):
    ...
    if not contents:
        ...
        log_table_l_c, log_table_c_l = calculate_log(prob_con_table, prob_lib_table)
        for key, value in sorted(log_table_l_c.iteritems(), key=lambda (k, v): (v, k), reverse=True):
            if iter <= 20:
                iter += 1
                print "%s %.04f" % (key, value)
        print
        iter = 1
        for key, value in sorted(log_table_c_l.iteritems(), key=lambda (k, v): (v, k), reverse=True):
            if iter <= 20:
                iter += 1
                print "%s %.04f" % (key, value)